...
class DocumentDataSource @Inject constructor() : IDocumentDataSource {
    override suspend fun addDocument(tenantID: String, uri: Uri, title: String): AddDocumentResult {
    ...
    override suspend fun removeDocument(document: Document): RemoveDocumentResult {
        try {
    ...
    override suspend fun updateDocument(document: Document): UpdateDocumentResult {
        try {
            fbUserDBTable?.child(FBKEY_TENANT)?.child(document.tenantID)?.child(
                FBKEY_DOCUMENT
            )?.child(document.id)
    ...
    override fun setDocumentsChangedListener(
        tenantID: String,
        listener: (List<Document>) -> Unit,
        errorListener: ((DatabaseError) -> Unit)?
    ) {
